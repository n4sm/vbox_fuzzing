...
void kafl_hprintf(const char *fmt, ...)
{
	...
	kAFL_hypercall(HYPERCALL_KAFL_NESTED_HPRINTF, (uintptr_t)virt_to_phys((void* )hprintf_buffer));
	va_end(args);
}
...
/*void pin_phys_page(phys_addr_t phys)
{
    struct page *page;
    ...
    SetPageReserved(page);    // mark as reserved (optional but common)
}*/
...
*/
	...
	//pin_phys_page((uint64_t)mmio);
	//pin_phys_page((uint64_t)mmio + 0x1000);
/*
	uint8_t __iomem *vmmio = ioremap(mmio, 0x2000);

	vmmio[0] = 0;
	vmmio[0x1000] = 0;

	if (mmio) {	
		payload_buffer[0] = (uint64_t)mmio;
		payload_buffer[1] = (uint64_t)mmio + 0x1000;
	}
*/
	/*
